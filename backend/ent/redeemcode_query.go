...
func (_q *RedeemCodeQuery) QuerySubscriptionPlan() *SubscriptionPlanQuery {
...
func (_q *RedeemCodeQuery) WithSubscriptionPlan(opts ...func(*SubscriptionPlanQuery)) *RedeemCodeQuery {
	query := (&SubscriptionPlanClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RedeemCodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RedeemCode, error) {
	...
	if query := _q.withSubscriptionPlan; query != nil {
		if err := _q.loadSubscriptionPlan(ctx, query, nodes, nil,
			func(n *RedeemCode, e *SubscriptionPlan) { n.Edges.SubscriptionPlan = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RedeemCodeQuery) loadSubscriptionPlan(ctx context.Context, query *SubscriptionPlanQuery, nodes []*RedeemCode, init func(*RedeemCode), assign func(*RedeemCode, *SubscriptionPlan)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*RedeemCode)
	for i := range nodes {
...
func (_q *RedeemCodeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSubscriptionPlan != nil {
			_spec.Node.AddColumnOnce(redeemcode.FieldSubscriptionPlanID)
		}
	}

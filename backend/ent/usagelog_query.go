...
func (_q *UsageLogQuery) QueryPlatform() *PlatformQuery {
...
func (_q *UsageLogQuery) WithPlatform(opts ...func(*PlatformQuery)) *UsageLogQuery {
	query := (&PlatformClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UsageLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UsageLog, error) {
	...
	if query := _q.withPlatform; query != nil {
		if err := _q.loadPlatform(ctx, query, nodes, nil,
			func(n *UsageLog, e *Platform) { n.Edges.Platform = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UsageLogQuery) loadPlatform(ctx context.Context, query *PlatformQuery, nodes []*UsageLog, init func(*UsageLog), assign func(*UsageLog, *Platform)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*UsageLog)
	for i := range nodes {
...
func (_q *UsageLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPlatform != nil {
			_spec.Node.AddColumnOnce(usagelog.FieldPlatformID)
		}
	}

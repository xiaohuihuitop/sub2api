...
// GetDashboardAccountSwitchSummary returns current selected account and recent real switch records.
// GET /api/v1/admin/ops/dashboard/account-switch-summary
func (h *OpsHandler) GetDashboardAccountSwitchSummary(c *gin.Context) {
	if h.opsService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Ops service not available")
		return
	}
	...
	data, err := h.opsService.GetAccountSwitchSummary(c.Request.Context(), filter, 30)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}

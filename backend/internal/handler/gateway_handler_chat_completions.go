...
// ChatCompletions handles the OpenAI Chat Completions API endpoint for Anthropic platform accounts.
...
func (e sub2APIMessagesExecutor) executeChatCompletions(c *gin.Context, usageSink gatewayruntime.UsageSink) {
	...
	if !modelTargetPlatformResolved(c, reqModel) {
		h.chatCompletionsErrorResponse(c, http.StatusBadRequest, "invalid_request_error", "Model is not supported by any authorized platform")
		return
	}
	...
	for {
		...
		selection, err := h.gatewayService.SelectAccountWithLoadAwareness(c.Request.Context(), service.PlatformSchedulingID(c.Request.Context()), selectionSessionHash, reqModel, fs.FailedAccountIDs, "", int64(0))
		if err != nil {

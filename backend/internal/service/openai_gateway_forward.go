...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	SetActualOpenAIUpstreamEndpoint(c, appendOpenAIResponsesRequestPathSuffix(openAIResponsesEndpoint, openAIResponsesRequestPathSuffix(c)))
	...
	if passthroughEnabled {
		...
		mappedModel := resolveOpenAIForwardModelWithContext(ctx, account, reqModel, "")
		passthroughBody := originalBody
		passthroughModel := reqModel
		if mappedModel != reqModel {
			passthroughBody = ReplaceModelInBody(passthroughBody, mappedModel)
	...
	billingModel := resolveOpenAIForwardModelWithContext(ctx, account, reqModel, "")
	...
	if !isCompactRequest && (codexImageGenerationBridgeEnabled || isOpenAIImageGenerationModel(requestView.Model) || openAIRequestBodyImageGenerationToolNeedsNormalization(body) || isOpenAIImageGenerationModel(upstreamModel)) {
		decoded, decodeErr := ensureReqBody()
		if decodeErr != nil {
			return nil, decodeErr
		}
	...
	} else if imageIntent && openAIRequestBodyHasImageGenerationDeclaration(body) {

...
func (a *Account) IsModelSupported(requestedModel string) bool {
	// Spark shadows are a structural account type. Their eligibility comes from
	// quota_dimension and the adapter's canonical Spark model aliases, never from
	// a generated credentials.model_mapping entry.
	if a != nil && a.IsShadow() && a.QuotaDimensionOrDefault() == QuotaDimensionSpark {
		return isCodexSparkModel(requestedModel)
	}

...
func (s *GatewayService) SelectAccount(ctx context.Context, platformID *int64, sessionHash string) (*Account, error) {
	return s.SelectAccountForModel(ctx, platformID, sessionHash, "")
}

// SelectAccountForModel 选择支持指定模型的账号（粘性会话+优先级+模型映射）
func (s *GatewayService) SelectAccountForModel(ctx context.Context, platformID *int64, sessionHash string, requestedModel string) (*Account, error) {
...
func (s *GatewayService) SelectAccountForModelWithExclusions(ctx context.Context, platformNamespaceID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	...
	account, err := s.selectAccountForModelWithPlatform(ctx, platformNamespaceID, sessionHash, requestedModel, excludedIDs, scope.AccountPlatform)
	if err != nil {
		return nil, err
	}
...
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, platformID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, sub2apiUserID int64) (*AccountSelectionResult, error) {
	// 调试日志：记录调度入口参数
	...
	slog.Debug("account_scheduling_starting",
		"platform_namespace_id", derefPlatformID(platformID),
	...
	if prefetch := prefetchedStickyAccountIDFromContext(ctx, platformID); prefetch > 0 {
		stickyAccountID = prefetch
		stickySource = "prefetch"
	} else if sessionHash != "" && s.cache != nil {
		if accountID, err := s.cache.GetSessionAccountID(ctx, derefPlatformID(platformID), sessionHash); err == nil {
	...
	if s.debugModelRoutingEnabled() && requestedModel != "" {
		logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] select entry: platform_id=%v platform=%s model=%s session=%s sticky_account=%d load_batch=%v concurrency=%v",
			derefPlatformID(platformID), scope.AccountPlatform, requestedModel, shortSessionHash(sessionHash), stickyAccountID, cfg.LoadBatchEnabled, s.concurrencyService != nil)
	}
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		...
		for {
			account, err := s.SelectAccountForModelWithExclusions(ctx, platformID, sessionHash, requestedModel, localExcluded)
			if err != nil {
				return nil, err
			}
	...
	if s.debugModelRoutingEnabled() && platform == PlatformAnthropic && requestedModel != "" {
		logger.LegacyPrintf("service.gateway", "[ModelRoutingDebug] load-aware enabled: platform_namespace_id=%v model=%s session=%s platform=%s", derefPlatformID(platformID), requestedModel, shortSessionHash(sessionHash), platform)
	}

	accounts, useMixed, err := s.listSchedulableAccounts(ctx, platformID, platform, false)
	if err != nil {
		return nil, err
	}
	if len(accounts) == 0 {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if ok {
				...
				if clearSticky {
					...
					_ = s.cache.DeleteSessionAccountID(ctx, derefPlatformID(platformID), sessionHash)
				}

				// accountByID 来自当前 Platform 账号池，无需再次检查归属。
				...
				if !clearSticky && platformOK && modelSupported && modelSchedulable && quotaOK && windowCostOK && rpmOK && schedulable {
					...
					if err == nil && result.Acquired {
						...
						if !s.checkAndRegisterSession(ctx, account, sessionHash) {
							...
							if s.cache != nil {
								_ = s.cache.RefreshSessionTTL(ctx, derefPlatformID(platformID), sessionHash, stickySessionTTL)
							}
							return s.newSelectionResult(ctx, account, true, result.ReleaseFunc, nil)
						}
					} else {
	...
	if err != nil {
		if result, ok, fallbackErr := s.tryAcquireByFallbackOrder(ctx, candidates, platformID, sessionHash, preferOAuth); fallbackErr != nil {
			...
			if err == nil && result.Acquired {
				...
				if !s.checkAndRegisterSession(ctx, selected.account, sessionHash) {
					...
					if sessionHash != "" && s.cache != nil {
						_ = s.cache.SetSessionAccountID(ctx, derefPlatformID(platformID), sessionHash, selected.account.ID, stickySessionTTL)
					}
...
func (s *GatewayService) tryAcquireByFallbackOrder(ctx context.Context, candidates []*Account, platformID *int64, sessionHash string, preferOAuth bool) (*AccountSelectionResult, bool, error) {
	...
	for _, acc := range ordered {
		...
		if err == nil && result.Acquired {
			...
			if sessionHash != "" && s.cache != nil {
				_ = s.cache.SetSessionAccountID(ctx, derefPlatformID(platformID), sessionHash, acc.ID, stickySessionTTL)
			}
...
func (s *GatewayService) listSchedulableAccounts(ctx context.Context, platformID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	if scope, scoped := PlatformSchedulingScopeFromContext(ctx); scoped {
		if !strings.EqualFold(strings.TrimSpace(platform), scope.AccountPlatform) {
			return nil, false, fmt.Errorf("%w: platform scope protocol %q does not match requested protocol %q", ErrPlatformInvalid, scope.AccountPlatform, platform)
		}
		lister, ok := s.accountRepo.(PlatformPoolAccountLister)
		if !ok {
			return nil, false, fmt.Errorf("%w: account repository cannot list platform pool accounts", ErrPlatformInvalid)
		}
		accounts, err := listPlatformPoolSchedulableAccounts(ctx, lister, scope)
		return accounts, false, err
	}
	return nil, false, fmt.Errorf("%w: platform scheduling scope is required", ErrPlatformInvalid)
}

// IsSingleAntigravityPlatformAccount 检查当前 Platform 是否只有一个 antigravity 可调度账号。
// 用于 Handler 层在首次请求时提前设置 SingleAccountRetry context，
// 避免单账号池收到 503 时错误地设置模型限流标记导致后续请求连续快速失败。
func (s *GatewayService) IsSingleAntigravityPlatformAccount(ctx context.Context, platformID *int64) bool {
	accounts, _, err := s.listSchedulableAccounts(ctx, platformID, PlatformAntigravity, true)
	if err != nil {
		return false
	}
...
func (s *GatewayService) accountMatchesCurrentPlatformPool(ctx context.Context, account *Account, _ *int64) bool {
	return accountMatchesPlatformSchedulingScope(ctx, account)
}
...
func (s *GatewayService) selectAccountForModelWithPlatform(ctx context.Context, platformID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, platform string) (*Account, error) {
		...
		if err == nil && accountID > 0 {
			if _, excluded := excludedIDs[accountID]; !excluded {
				...
				if err == nil {
					...
					if !clearSticky && s.accountMatchesCurrentPlatformPool(ctx, account, platformID) && account.Platform == platform && (requestedModel == "" || s.isModelSupportedByAccountWithContext(ctx, account, requestedModel)) && s.isAccountSchedulableForModelSelection(ctx, account, requestedModel) && s.isAccountSchedulableForQuota(account) && s.isAccountSchedulableForWindowCost(ctx, account, true) && s.isAccountSchedulableForRPM(ctx, account, true) {
						return account, nil
					}
				}
			}
	...
	var err error
	accounts, _, err = s.listSchedulableAccounts(ctx, platformID, platform, hasForcePlatform)
	if err != nil {
	...
	if selected == nil {
		stats := s.logDetailedSelectionFailure(ctx, platformID, sessionHash, requestedModel, platform, accounts, excludedIDs, false)
		if requestedModel != "" {
	...
	// 4. 建立粘性绑定
	if sessionHash != "" && s.cache != nil {
		if err := s.cache.SetSessionAccountID(ctx, derefPlatformID(platformID), sessionHash, selected.ID, stickySessionTTL); err != nil {
...
func (s *GatewayService) logDetailedSelectionFailure(
	...
	logger.LegacyPrintf(
		"service.gateway",
		"[SelectAccountDetailed] platform_namespace_id=%v model=%s platform=%s session=%s total=%d eligible=%d excluded=%d unschedulable=%d platform_filtered=%d model_unsupported=%d model_rate_limited=%d sample_platform_filtered=%v sample_model_unsupported=%v sample_model_rate_limited=%v",
		derefPlatformID(platformID),
		requestedModel,
		platform,

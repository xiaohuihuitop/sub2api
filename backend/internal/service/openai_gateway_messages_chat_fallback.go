...
func (s *OpenAIGatewayService) streamChatCompletionsAsAnthropic(
	...
	for scanner.Scan() {
		...
		if firstTokenMs == nil && !isOpenAIChatUsageOnlyStreamChunk(payload) && chatChunkStartsMessagesOutput(&chunk) {
...
func chatChunkStartsMessagesOutput(chunk *apicompat.ChatCompletionsChunk) bool {
	if chunk == nil {
		return false
	}

...
func TestUsageLogRepositoryCreateSyncRequestTypeAndLegacyFields(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO usage_logs").
		WithArgs(
			...
			sqlmock.AnyArg(), // platform_id
			sqlmock.AnyArg(), // billing_source_type
			log.InputTokens,
			log.OutputTokens,
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullString{Valid: true, String: "subscription"},
			0, 0, 0, 0, 0, 0,
		...
		require.Equal(t, int64(44), *log.PlatformID)
		require.NotNil(t, log.BillingSourceType)
	...
	t.Run("request_type_ws_v2_overrides_legacy", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullInt64{},   // platform_id
			sql.NullString{},  // billing_source_type
			1,                 // input_tokens
			2,                 // output_tokens

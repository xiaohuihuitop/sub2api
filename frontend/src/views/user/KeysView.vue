      ...
      <template #table>
        ...
        >
          ...
          <template #cell-group="{ row }">
            <button
              @click="editKey(row)"
              class="-mx-2 -my-1 flex w-full flex-wrap items-center gap-2 rounded-lg px-2 py-1 text-left transition-all duration-200 hover:bg-gray-100 dark:hover:bg-dark-700"
              :title="t('keys.clickToChangeGroup')"
            >
              <template v-if="(row.groups && row.groups.length > 0) || row.group">
                <GroupBadge
                  v-for="group in (row.groups && row.groups.length > 0 ? row.groups : (row.group ? [row.group] : []))"
                  :key="group.id"
                  :name="group.name"
    ...
    >
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div class="space-y-3">
          ...
          <div class="max-h-72 space-y-2 overflow-y-auto rounded-xl border border-gray-200 bg-white p-2 dark:border-dark-600 dark:bg-dark-800" data-tour="key-form-group">
            <label
            ...
            >
              <input
                ...
                :value="option.value"
                type="checkbox"
                class="mt-1 h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-500 dark:bg-dark-900"
              />
              <GroupOptionItem
                :name="option.label"

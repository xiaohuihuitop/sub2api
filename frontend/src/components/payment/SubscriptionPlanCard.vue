<template>
  ...
  >
    ...
    <div class="h-1.5 bg-primary-500 dark:bg-primary-400" />
      ...
      <div class="mb-3 flex items-start justify-between gap-2">
        ...
        <div class="shrink-0 text-right">
          ...
          <div v-if="plan.original_price" class="mt-0.5 flex items-center justify-end gap-1.5">
            ...
            <span class="rounded bg-primary-50 px-1 py-0.5 text-[10px] font-semibold text-primary-700 dark:bg-primary-900/30 dark:text-primary-300">{{ discountText }}</span>
          </div>
        </div>
      </div>
...
const rateDisplay = computed(() => {
  const rate = props.plan.rate_multiplier
